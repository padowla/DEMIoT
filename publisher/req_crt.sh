...
CERT_FILE="publisher.crt" # Define the path client certificate
CSR_FILE="publisher.csr" # Define the path client Certificate Signing Request
PRIVATE_KEY="publisher.key" # Define the path client private key
...
request_x509_certificate() {
	...
	# Create a CSR
	openssl req -new -out $CSR_FILE -newkey rsa:2048 -nodes -sha256 -keyout $PRIVATE_KEY -subj "/CN=publisher"
