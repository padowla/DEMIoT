...
CERT_FILE="subscriber.crt" # Define the path client certificate
CSR_FILE="subscriber.csr" # Define the path client Certificate Signing Request
PRIVATE_KEY="subscriber.key" # Define the path client private key
...
EJBCA_CERTIFICATE_PROFILE_NAME="ENDUSER" # Define the certificate profile name EJBCA
...
request_x509_certificate() {
	echo "[-] Certificate file not found, subscriber requesting certificate..."
	# Create a CSR
	openssl req -new -out $CSR_FILE -newkey rsa:2048 -nodes -sha256 -keyout $PRIVATE_KEY -subj "/CN=subscriber"
